...
export function GeneratorClient() {
  ...
  const cliCommand = useMemo(() => {
    ...
    return `npx shadcn@latest add ${origin}/api/replica/generator?${params.toString()}`
  }, [origin, requestedUrl])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/replica/generator?${params.toString()}`)
      if (!response.ok) {
      ...
      <Card>
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            ...
            generate a static JSX snapshot of the GithubReplica output you can ship without runtime
            fetching.
          </CardDescription>
        </CardHeader>
            ...
            <Card className="border-dashed">
              <CardHeader className="space-y-2">
                ...
                <CardDescription className="space-y-1 text-sm">
                  ...
                  <p className="text-muted-foreground">File: Replica/Github/{componentName}.tsx</p>
                  {slug ? (
                    <p className="text-muted-foreground">Registry slug: github-replica-{slug}</p>
                  ) : null}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <PreviewTabs
                    title="GithubReplica snapshot"
                    preview={<GithubReplicaDisplay resource={resource} className="text-sm" />}
                    code={code}
                    language="tsx"
                  />
                </div>
