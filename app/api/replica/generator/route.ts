...
export async function GET(req: NextRequest) {
  ...
  const registry = {
    ...
    files: [
      {
        type: "registry:component",
        path: `Replica/Github/${componentName}.tsx`,
...
async function buildSnapshot(context: SnapshotContext): Promise<SnapshotPayload> {
  ...
  const lines = [
    ...
    "",
    `export const githubReplicaData = ${dataLiteral} as const`,
    "",
    "export type GithubReplicaData = typeof githubReplicaData",
    "",
    `export function ${context.componentName}() {`,
    `  return <${config.exportName} data={githubReplicaData} />`,
    "}",
    "",
  ]
...
> = {
  pull: {
    file: "components/Replica/Github/views/pull-replica.tsx",
    exportName: "GithubPullReplica",
    registryDependencies: ["tooltip"],
  },
  issue: {
    file: "components/Replica/Github/views/issue-replica.tsx",
    exportName: "GithubIssueReplica",
    registryDependencies: ["tooltip"],
  },
  repo: {
    file: "components/Replica/Github/views/repo-replica.tsx",
    exportName: "GithubRepoReplica",
    registryDependencies: ["avatar", "badge", "tooltip"],
  },
  user: {
    file: "components/Replica/Github/views/user-replica.tsx",
    exportName: "GithubUserReplica",
...
function buildComponentName(slug: string) {
  ...
  const name = `GithubReplica${suffix}`
  if (/^[A-Za-z]/.test(name)) return truncate(name, 64)
  return "GithubReplicaSnapshot"
}
