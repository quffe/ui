// Derived from https://github.com/github-linguist/linguist/blob/main/lib/linguist/languages.yml
...
const normalizedLanguageColors: Record<string, string> = Object.fromEntries(
  Object.entries(githubLanguageColors).map(([name, color]) => [name.toLowerCase(), color])
)

export function getGithubLanguageColor(language?: string | null): string | null {
  if (!language) return null
  const trimmed = language.trim()
  if (!trimmed) return null
  return githubLanguageColors[trimmed] ?? normalizedLanguageColors[trimmed.toLowerCase()] ?? null
}
